...
const VatFilter: React.FC<YearMonthSelectorProps> = ({
  years,
  fetchIncomesData,
  loading,
  refetch,
    ...
    <div className="flex flex-col gap-4 md:flex-row md:items-center my-12">
      ...
      <div className="w-full md:w-1/3">
        <select
          ...
          className={`w-full p-3 border border-gray-300 rounded-md ${!selectedYear ? "cursor-not-allowed" : ""}`}
        >
          <option value="" disabled>
            Select Month
          </option>
      ...
      <RevButton name="Search" onClick={handleSearch} />
    </div>
  );
};
