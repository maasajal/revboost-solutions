...
const TaxTable = () => {
  ...
  const getTaxData = async () => {
    try {
      setLoading(true);
      const res = await axiosSecure.post(`/incomes/tax-status`, {
        name: "rajiul",
      });
      setTax(res.data);
      setLoading(false);
              ...
              {tax.map((income) => {
                return (
                  <tr>
                    ...
                    <td
                      className={`${income.vat_status === "pending" ? "text-primary" : "text-green-600"} border px-4 py-2`}
                    >
                      {income.totalIncome}
                    </td>
                    <td className="border px-4 py-2">{income.tax_rate}%</td>
                    <td className="border px-4 py-2">
                      {<p>${income.tax_amount}</p>}
                    </td>
